...
def check_alive(snake, grid_size):
    ...
    """
    snake_head = snake[0]
    touching_wall = any(ordinate <= 0 or ordinate >= grid_size - 1 for ordinate in snake_head)
    if touching_wall: return False
    
    for snake_body_part in snake[1:]:
        if snake_head[0] == snake_body_part[0] and snake_head[1] == snake_body_part[1]:
            return False
